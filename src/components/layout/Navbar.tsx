        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative" ref={menuRef}>
            <button
              ...
              className="flex h-10 w-10 items-center justify-center rounded-full bg-neutral-900 text-white shadow-sm transition hover:bg-neutral-700 dark:bg-white dark:text-neutral-900 dark:hover:bg-neutral-200"
            >
              {menuOpen ? (
                <Menu size={18} />
              ) : user ? (
            ...
            {menuOpen && (
              <div className="absolute right-0 top-14 z-50 w-72 overflow-hidden rounded-2xl border border-neutral-200 bg-white shadow-xl dark:border-neutral-700 dark:bg-neutral-900">
                ...
                <div className="border-b border-neutral-100 p-2 dark:border-neutral-800 md:hidden">
                  {NAV_LINKS.map((label) => (
                    <NavLink
                      ...
                      className={({ isActive }) =>
                        `flex w-full rounded-xl px-4 py-3 text-sm font-semibold transition ${
                          isActive
                ...
                {user ? (
                  ...
                  <div className="p-2">
                    <Link to="/login" onClick={() => setMenuOpen(false)} className="flex w-full items-center rounded-xl px-4 py-3 text-sm font-semibold text-neutral-700 transition hover:bg-neutral-100 dark:text-neutral-300 dark:hover:bg-neutral-800">
                      Login
                    </Link>
                    <Link to="/register" onClick={() => setMenuOpen(false)} className="flex w-full items-center rounded-xl px-4 py-3 text-sm font-semibold text-neutral-700 transition hover:bg-neutral-100 dark:text-neutral-300 dark:hover:bg-neutral-800">
                      Create an account
                    </Link>
                  </div>
                )}
