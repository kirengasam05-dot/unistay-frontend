        ...
        <div className="grid gap-6 lg:grid-cols-3">
          {courses.map(c => (
            <div key={c.id} className="overflow-hidden rounded-2xl border border-neutral-200 bg-white shadow-card dark:border-neutral-800 dark:bg-neutral-900">
              ...
              <div className="p-5">
                {c.category && (
                  <span className="rounded-full bg-neutral-100 px-3 py-1 text-xs font-bold text-neutral-700 dark:bg-neutral-800 dark:text-neutral-300">{c.category}</span>
                )}
                <h2 className="mt-3 text-lg font-black text-neutral-900 dark:text-white">{c.title}</h2>
                <p className="mt-1 text-sm text-neutral-500 dark:text-neutral-400">
                  {c.materials ? `${c.materials} materials` : 'Learning materials'}
                ...
                <Link to="/student/assignments" className="btn-black mt-5 w-full rounded-xl text-center block">
                  Continue course
                </Link>
              </div>
            </div>
