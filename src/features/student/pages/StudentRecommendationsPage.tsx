  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setHousing(all.filter((h) => h.verificationStatus === "VERIFIED" && h.availability));
      } catch (err) {
        ...
        <div className="mt-5 space-y-3">
          {topJobs.map((j) => (
            <div key={j.id} className="flex flex-col gap-4 rounded-2xl border border-neutral-200 bg-white p-5 dark:border-neutral-800 dark:bg-neutral-900 md:flex-row md:items-center md:justify-between">
              <div>
                <span className="rounded-full bg-neutral-100 px-3 py-1 text-xs font-bold text-neutral-600 dark:bg-neutral-800 dark:text-neutral-400">{j.category}</span>
                ...
                <div className="mt-1 flex flex-wrap gap-4 text-sm text-neutral-500">
                  ...
                  <span className="flex items-center gap-1"><Wallet size={14} />{money(j.salary)}</span>
                </div>
              </div>
