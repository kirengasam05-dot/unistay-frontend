      ...
      <section className="page-hero">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-8 grid gap-2 rounded-2xl bg-white p-2 sm:grid-cols-[1fr_180px_auto]">
            ...
            <button className="btn-black rounded-xl px-5">Search</button>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-4 py-14 sm:px-6">
          ...
          {categories.map((cat) => (
            <button key={cat} onClick={() => setKeyword(cat)}
              ...
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-neutral-100 dark:bg-neutral-800">
                <Briefcase size={17} className="text-neutral-700 dark:text-neutral-300" />
              </div>
              <div>
        ...
        <div className="mt-10 space-y-4">
          {filtered.map((j) => {
            ...
            return (
              <div key={j.id} className="rounded-2xl border border-neutral-200 bg-white p-6 shadow-card transition hover:shadow-md dark:border-neutral-800 dark:bg-neutral-900">
                <div className="flex flex-col gap-5 md:flex-row md:items-start md:justify-between">
                  <div className="flex-1">
                    ...
                    <p className="mt-0.5 text-sm font-semibold text-neutral-500 dark:text-neutral-400">{j.company}</p>
                    <div className="mt-3 flex flex-wrap gap-4 text-sm text-neutral-500 dark:text-neutral-400">
                      ...
                      <span className="flex items-center gap-1.5"><Wallet size={13} />RWF {j.salary.toLocaleString()}</span>
                      <span className="flex items-center gap-1.5"><Clock3 size={13} />Deadline {j.deadline}</span>
                    </div>
                  ...
                  <div className="shrink-0">
                    ...
                    <p className="mt-2 text-center text-xs text-neutral-400">Employer reviews compatibility</p>
                  </div>
                </div>
              </div>
            );
          ...
          {filtered.length === 0 && (
            <div className="py-16 text-center">
              <p className="font-black text-neutral-900 dark:text-white">No jobs found</p>
