...
const hasOpenBeds = (housing: Housing) => housing.occupancyStatus ? housing.occupancyStatus === 'AVAILABLE' : (housing.available ?? housing.availability ?? false);
        ...
        <section className="mx-auto max-w-7xl px-6 py-16">
          ...
          <div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {housing.slice(0, 6).map(item => {
              const isAvailable = hasOpenBeds(item);
              return (
              <Link key={item.id} to={`/hostels/${item.id}`} className="group overflow-hidden rounded-lg border border-neutral-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-lg dark:border-neutral-800 dark:bg-neutral-900">
                <div className="relative"><img src={image(item)} alt={item.title} className="h-52 w-full object-cover transition duration-500 group-hover:scale-105" /><span className={`absolute left-4 top-4 rounded-full px-3 py-1 text-xs font-black text-white ${isAvailable ? 'bg-emerald-600' : 'bg-red-600'}`}>{isAvailable ? 'Available' : 'Occupied all'}</span></div>
