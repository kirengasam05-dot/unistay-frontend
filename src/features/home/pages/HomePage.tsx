...
import { ArrowRight, Briefcase, CheckCircle2, Clock3, GraduationCap, MapPin, Search, ShieldCheck, Users, Wallet } from 'lucide-react';
...
export default function HomePage() {
  ...
  useEffect(() => {
    housingApi.getAll().then(all => setLiveHousing(all.filter(h => h.verificationStatus === 'VERIFIED'))).catch(() => {});
    jobsApi.getAll().then(setJobs).catch(() => {});
    coursesApi.getAll().then(setCourses).catch(() => {});
  }, []);

  const featured    = useMemo(() => liveHousing.slice(0, 2), [liveHousing]);
    ...
    <div className="bg-white dark:bg-neutral-950">
      <section className="relative overflow-hidden border-b border-neutral-200 bg-neutral-950 text-white dark:border-neutral-800">
        ...
        <div className="relative mx-auto grid max-w-7xl gap-12 px-6 py-20 lg:grid-cols-[1.05fr_.95fr] lg:items-center">
          <div>
            ...
            <h1 className="mt-6 max-w-4xl text-5xl font-black leading-tight tracking-tight md:text-7xl">Find housing, jobs and skills that fit your student life.</h1>
            <p className="mt-6 max-w-2xl text-lg text-neutral-300">One student profile to book verified housing, apply for internships, complete courses and earn certificates that boost your job match score.</p>
            <div className="mt-8 grid gap-3 rounded-[2rem] border border-white/10 bg-white p-3 text-black shadow-2xl md:grid-cols-[1fr_1fr_auto]">
              ...
              <button onClick={runSearch} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-black px-6 py-3 font-bold text-white hover:bg-neutral-800">Search <ArrowRight size={18} /></button>
            </div>
      ...
      <section className="mx-auto max-w-7xl px-6 py-16">
        ...
        <div className="mt-8 grid gap-5 md:grid-cols-4">
          {staticCategories.map(({ name, icon: Icon }) => (
      ...
      {isLive && (
        <section className="mx-auto max-w-7xl px-6 py-16">
          <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
            ...
            <Link to="/housing" className="inline-flex items-center gap-2 font-bold text-neutral-900 underline dark:text-white">Browse all <ArrowRight size={16} /></Link>
          </div>
            ...
            {housingList.map(h => (
              <Link key={h.id} to={`/housing/${h.id}`} className="group overflow-hidden rounded-[2rem] border border-neutral-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-lg dark:border-neutral-800 dark:bg-neutral-900">
                <div className="relative">
                  ...
                  <span className={`absolute left-4 top-4 rounded-full px-3 py-1 text-xs font-black text-white ${h.availability ? 'bg-emerald-600' : 'bg-neutral-500'}`}>{h.availability ? 'Available' : 'Booked'}</span>
                </div>
                <div className="p-5">
      ...
      <section className="border-y border-neutral-200 bg-neutral-50 px-6 py-20 dark:border-neutral-800 dark:bg-neutral-900">
        <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[.9fr_1.1fr] lg:items-center">
          <div>
            ...
            <Link to="/housing" className="mt-8 inline-flex items-center gap-2 rounded-2xl bg-black px-6 py-3 font-bold text-white dark:bg-white dark:text-neutral-900">Explore verified housing <ArrowRight size={18} /></Link>
          </div>
          <div className="grid gap-4 md:grid-cols-2">
            {featured.map(h => (
              <Link key={h.id} to={`/housing/${h.id}`} className="rounded-[2rem] bg-white p-3 shadow-sm ring-1 ring-neutral-200 transition hover:-translate-y-1 hover:shadow-lg dark:bg-neutral-800 dark:ring-neutral-700">
              ...
              <Card key={j.id} className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
                <div>
                  ...
                  <div className="mt-2 flex flex-wrap gap-4 text-sm text-neutral-600 dark:text-neutral-400">
                    <span className="flex items-center gap-1"><MapPin size={16} />{j.location}</span>
                    {j.deadline && <span className="flex items-center gap-1"><Clock3 size={16} />{j.deadline}</span>}
                    {j.salary  && <span className="flex items-center gap-1"><Wallet size={16} />{money(j.salary)}</span>}
                  </div>
                </div>
            ...
            <div className="grid gap-4 md:grid-cols-3 lg:col-span-2">
              {courses.slice(0, 3).map(c => (
                <div key={c.id} className="rounded-[2rem] bg-white p-4 text-black">
                  {c.thumbnail && <img className="h-32 w-full rounded-2xl object-cover" src={c.thumbnail} alt={c.title} />}
                  <h3 className="mt-4 font-black">{c.title}</h3>
                  {c.category && <p className="mt-1 text-sm text-neutral-600">{c.category}</p>}
                  {c.progress !== undefined && <div className="mt-4 h-2 rounded-full bg-neutral-100"><div className="h-2 rounded-full bg-black" style={{ width: `${c.progress}%` }} /></div>}
                </div>
              ))}
            </div>
          </div>
