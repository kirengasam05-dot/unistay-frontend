...
import { Briefcase, Building2, CheckCircle2, Clock3, GraduationCap, MapPin, Search, ShieldCheck, Users, Wallet } from 'lucide-react';
              ...
              {housings.slice(0, 2).map(h => (
                <div key={h.id} className="rounded-[2rem] bg-white p-3 shadow-sm ring-1 ring-neutral-200 dark:bg-neutral-800 dark:ring-neutral-700">
                  {(h.image || (h.images && h.images[0])) && (
                    <img className="h-48 w-full rounded-[1.4rem] object-cover" src={h.image || h.images![0]} alt={h.title} />
                  )}
                  <div className="p-4">
              ...
              <Card key={j.id} className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
                <div>
                  ...
                  <div className="mt-2 flex flex-wrap gap-4 text-sm text-neutral-600 dark:text-neutral-400">
                    ...
                    {j.salary && <span className="flex items-center gap-1"><Wallet size={16} />RWF {j.salary.toLocaleString()}</span>}
                  </div>
                </div>
        ...
        <section className="bg-neutral-950 px-6 py-20 text-white">
          <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-3">
            <div className="lg:col-span-1">
              <h2 className="text-4xl font-black">Study. Take exam. Earn certificate.</h2>
              <p className="mt-3 text-neutral-300">Admin publishes courses, videos and exams. Students complete them to unlock certificates and improve profile strength.</p>
            </div>
                ...
                <div key={c.id} className="rounded-[2rem] bg-white p-4 text-black">
                  ...
                  {c.progress !== undefined && (
                    <div className="mt-4 h-2 rounded-full bg-neutral-100">
                      <div className="h-2 rounded-full bg-black" style={{ width: `${c.progress}%` }} />
                    </div>
                  )}
                </div>
              ))}
