...
function Field({
  label, value, onChange, placeholder, icon: Icon, type = 'text', error,
}: {
  label: string;
  value: string;
...
export default function ProfilePage() {
  ...
  const [form, setForm] = useState<FormState>({
    ...
    bio:           stored?.bio           ?? '',
    skillsProfile: stored?.skillsProfile ?? '',
  });
  ...
  function flash(setter: (v: boolean) => void) {
    ...
    timerRef.current = setTimeout(() => setter(false), 3000);
  }
  ...
  function saveInfo() {
    ...
    if (!stored) return;
    const updated: AuthUser = { ...stored, ...form };
    saveUser(updated);
    setErrors({});
    flash(setSaved);
  }
  ...
  if (!stored) {
    return (
      <div className="card py-16 text-center">
        <p className="font-black text-neutral-900 dark:text-white">Not logged in</p>
        <p className="mt-1 text-sm text-neutral-500 dark:text-neutral-400">Please log in to view your profile.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card">
        <div className="flex flex-wrap items-center gap-5">
          <div className="relative shrink-0">
            <div className={`flex h-20 w-20 items-center justify-center rounded-2xl text-2xl font-black text-white ${avatarBg[role] ?? 'bg-neutral-700'}`}>
              {initials}
            </div>
            <button className="absolute -bottom-1 -right-1 flex h-7 w-7 items-center justify-center rounded-full border-2 border-white bg-neutral-900 text-white transition hover:bg-neutral-700 dark:border-neutral-900 dark:bg-white dark:text-neutral-900 dark:hover:bg-neutral-200">
              <Camera size={13} />
            </button>
          </div>
          <div className="flex-1 min-w-0">
            ...
            <span className={`mt-2 inline-block rounded-full px-3 py-1 text-xs font-bold ${roleColors[role] ?? ''}`}>
      ...
      <div className="card">
        ...
        {/* Skills — student + employer only */}
        {(role === 'STUDENT' || role === 'EMPLOYER') && (
          <div className="mt-4">
            <label className="block text-sm font-semibold text-neutral-700 dark:text-neutral-300 mb-1.5">
              {role === 'STUDENT' ? 'Skills profile' : 'Industry / expertise'}
            </label>
            <div className="relative">
              <Sparkles size={15} className="pointer-events-none absolute left-3.5 top-1/2 -translate-y-1/2 text-neutral-400" />
              <input
                value={form.skillsProfile}
                onChange={e => setForm({ ...form, skillsProfile: e.target.value })}
                placeholder={role === 'STUDENT' ? 'e.g. React, Communication, Problem-solving' : 'e.g. Fintech, Software Development'}
                className="input pl-10"
              />
            </div>
            {role === 'STUDENT' && (
              <p className="mt-1.5 text-xs text-neutral-500 dark:text-neutral-400">
                Listed skills improve your job compatibility score.
              </p>
            )}
          </div>
        )}
        ...
        <div className="mt-6 flex items-center gap-3">
          <button onClick={saveInfo} className="btn-black rounded-xl">
            Save changes
          </button>
          {saved && (
      ...
      <div className="card">
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-3">
          {(['current', 'next', 'confirm'] as const).map((key) => (
            <div key={key}>
              <label className="block text-sm font-semibold text-neutral-700 dark:text-neutral-300 mb-1.5">
                {key === 'current' ? 'Current password' : key === 'next' ? 'New password' : 'Confirm new'}
              </label>
              <div className="relative">
        ...
        <div className="mt-6 flex items-center gap-3">
          <button onClick={savePassword} className="btn-black rounded-xl">
            Update password
          </button>
          {pwSaved && (
            <span className="flex items-center gap-1.5 text-sm font-semibold text-emerald-600 dark:text-emerald-400">
      ...
      <div className="card">
        ...
        <p className="mt-1 text-sm text-neutral-500 dark:text-neutral-400">System information about your account.</p>

        <dl className="mt-5 grid gap-3 sm:grid-cols-2">
          {[
            { label: 'Account ID', value: stored.id },
