...
export default function ResetPasswordPage() {
  ...
  const inputClass = 'w-full rounded-lg border border-neutral-200 bg-white px-4 py-3 pr-12 text-sm text-neutral-900 placeholder-neutral-400 outline-none transition focus:border-neutral-500 dark:border-white/10 dark:bg-white/10 dark:text-white dark:placeholder-neutral-500 dark:focus:border-white/30 dark:focus:bg-white/15';
        ...
        <div className="w-full rounded-2xl border border-neutral-200 bg-white p-8 shadow-xl dark:border-white/10 dark:bg-white/[0.07] dark:shadow-2xl dark:backdrop-blur-2xl">
          {!token ? (
            <div className="text-center">
              <div className="mx-auto grid h-14 w-14 place-items-center rounded-2xl bg-red-100 text-red-600 dark:bg-red-500/15 dark:text-red-400">
                <ShieldAlert size={26} />
              </div>
              <h2 className="mt-5 text-2xl font-black text-neutral-900 dark:text-white">Invalid reset link</h2>
              <p className="mt-2 text-sm text-neutral-500 dark:text-neutral-400">This link is missing its reset token or has expired. Request a new one.</p>
              <Link to="/forgot-password" className="mt-7 inline-block rounded-lg bg-neutral-900 px-5 py-3 text-sm font-black text-white dark:bg-white dark:text-neutral-900">Request new link</Link>
            </div>
          ) : (
            <form onSubmit={submit}>
              <h2 className="text-2xl font-black text-neutral-900 dark:text-white">Set a new password</h2>
              <p className="mt-1 text-sm text-neutral-500 dark:text-neutral-400">Choose a strong password you haven't used before.</p>
              ...
              <div className="mt-6 space-y-4">
                <div className="space-y-1.5">
                  <label className="text-xs font-semibold uppercase tracking-widest text-neutral-500 dark:text-neutral-400">New password</label>
                  <div className="relative">
                    <input value={password} onChange={e => setPassword(e.target.value)} placeholder="••••••••" type={showPw ? 'text' : 'password'} required className={inputClass} />
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-semibold uppercase tracking-widest text-neutral-500 dark:text-neutral-400">Confirm new password</label>
                  <input value={confirm} onChange={e => setConfirm(e.target.value)} placeholder="••••••••" type={showPw ? 'text' : 'password'} required className={inputClass.replace('pr-12', '')} />
                </div>
