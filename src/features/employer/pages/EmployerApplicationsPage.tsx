...
export default function EmployerApplicationsPage() {
  ...
  async function decide(id: string, action: 'accept' | 'reject') {
    ...
    try {
      ...
      toast.success(action === 'accept' ? 'Application accepted. Acceptance email generated.' : 'Application rejected. Rejection email sent with improvement advice.');
    } catch (err: any) {
            ...
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-3 text-sm font-bold">
                  Email:{' '}
                  {a.status === 'ACCEPTED' ? 'Acceptance email ready'
                    : a.status === 'REJECTED' ? 'Rejection email sent with improvement advice'
              ...
              {a.status === 'PENDING' && (
                <div className="flex gap-2">
                  <button
                    disabled={busyId === a.id || a.compatible === false}
                    onClick={() => decide(a.id, 'accept')}
                    className="rounded-2xl bg-green-600 px-4 py-2 font-bold text-white disabled:bg-neutral-300 flex items-center gap-2"
                  >
                  ...
                  <button
                    ...
                    className="rounded-2xl bg-red-600 px-4 py-2 font-bold text-white disabled:bg-neutral-300 flex items-center gap-2"
                  >
                    {busyId === a.id && <Loader2 size={14} className="animate-spin" />}
                    Reject
                  </button>
