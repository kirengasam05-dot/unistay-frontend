...
import { Briefcase, Calendar, ChevronDown, Clock3, Loader2, MapPin, Pencil, Trash2, User, X } from 'lucide-react';
        ...
        <div className="flex items-start justify-between gap-4 border-b border-neutral-100 px-6 py-5 dark:border-neutral-800">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              <span className={`rounded-full px-2.5 py-0.5 text-xs font-black ${SCHEDULE_COLOR[job.scheduleType] ?? 'bg-neutral-100 text-neutral-600'}`}>
                {SCHEDULE_LABELS[job.scheduleType] ?? job.scheduleType}
              </span>
              {job.category && (
            ...
            <h2 className="mt-2 text-xl font-black text-neutral-900 dark:text-white">{job.title}</h2>
            {job.company && (
              <p className="mt-0.5 text-sm text-neutral-500 dark:text-neutral-400">{job.company}</p>
            )}
          </div>
          <button
            onClick={onClose}
            className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-neutral-200 text-neutral-400 hover:bg-neutral-100 dark:border-neutral-700 dark:hover:bg-neutral-800"
          >
            <X size={15} />
          </button>
        </div>
        ...
        <div className="flex-1 space-y-6 overflow-y-auto p-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="rounded-2xl bg-neutral-50 p-4 dark:bg-neutral-800">
              ...
              <div className="mt-1.5 flex items-center gap-1.5 text-sm font-semibold text-neutral-900 dark:text-white">
                <MapPin size={14} className="shrink-0 text-neutral-400" />
                {job.location || '—'}
              </div>
            </div>
            <div className="rounded-2xl bg-neutral-50 p-4 dark:bg-neutral-800">
              <p className="text-xs font-bold uppercase tracking-widest text-neutral-400">Deadline</p>
              <div className="mt-1.5 flex items-center gap-1.5 text-sm font-semibold text-neutral-900 dark:text-white">
                <Clock3 size={14} className="shrink-0 text-neutral-400" />
                {job.deadline || 'No deadline'}
              </div>
            </div>
            {job.salary && (
              <div className="rounded-2xl bg-neutral-50 p-4 dark:bg-neutral-800">
                <p className="text-xs font-bold uppercase tracking-widest text-neutral-400">Salary</p>
                <p className="mt-1.5 text-sm font-semibold text-neutral-900 dark:text-white">
                  RWF {job.salary.toLocaleString()}
                </p>
              </div>
            )}
            {job.createdAt && (
              <div className="rounded-2xl bg-neutral-50 p-4 dark:bg-neutral-800">
                <p className="text-xs font-bold uppercase tracking-widest text-neutral-400">Posted</p>
                <p className="mt-1.5 text-sm font-semibold text-neutral-900 dark:text-white">
          ...
          {job.description ? (
            <div>
              <p className="mb-2 text-xs font-bold uppercase tracking-widest text-neutral-400">Description</p>
              <p className="whitespace-pre-wrap text-sm leading-relaxed text-neutral-700 dark:text-neutral-300">
                {job.description}
              </p>
            </div>
          ) : (
            <div className="rounded-2xl border border-dashed border-neutral-200 py-6 text-center dark:border-neutral-700">
          ...
          {job.requiredSkills && job.requiredSkills.length > 0 && (
            <div>
              <p className="mb-3 text-xs font-bold uppercase tracking-widest text-neutral-400">Required Skills</p>
              <div className="flex flex-wrap gap-2">
                {job.requiredSkills.map(s => (
                  <span key={s} className="rounded-xl bg-neutral-100 px-3 py-1.5 text-sm font-semibold text-neutral-700 dark:bg-neutral-800 dark:text-neutral-300">
                    {s}
                  </span>
                ))}
              </div>
          ...
          <div className="rounded-xl bg-neutral-50 px-4 py-3 dark:bg-neutral-800">
            <p className="text-xs font-bold text-neutral-400">Job ID</p>
            <p className="mt-0.5 font-mono text-xs text-neutral-500">{job.id}</p>
          </div>
        </div>

        {/* Footer */}
        <div className="flex gap-3 border-t border-neutral-100 p-5 dark:border-neutral-800">
          <button
            onClick={() => { onEdit(job); onClose(); }}
            className="flex flex-1 items-center justify-center gap-2 rounded-xl border border-neutral-200 py-2.5 text-sm font-bold text-neutral-700 transition hover:bg-neutral-50 dark:border-neutral-700 dark:text-neutral-300 dark:hover:bg-neutral-800"
          >
            <Pencil size={15} /> Edit
          </button>
          <button
            ...
            disabled={deleting}
            className="flex flex-1 items-center justify-center gap-2 rounded-xl border border-red-200 bg-red-50 py-2.5 text-sm font-bold text-red-600 transition hover:bg-red-100 disabled:opacity-50 dark:border-red-800/40 dark:bg-red-900/20 dark:text-red-400"
          >
    ...
    <div className="space-y-8">
          ...
          <div className="divide-y divide-neutral-100 dark:divide-neutral-800">
            {jobs.map(j => (
              <div
                ...
                className={`flex cursor-pointer flex-wrap items-start justify-between gap-4 px-6 py-4 transition hover:bg-neutral-50 dark:hover:bg-neutral-800/30 ${editingId === j.id ? 'bg-neutral-50 dark:bg-neutral-800/50' : ''} ${selectedJob?.id === j.id ? 'ring-inset ring-2 ring-neutral-900 dark:ring-white' : ''}`}
              >
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="mt-0.5 text-sm text-neutral-500 dark:text-neutral-400">
                      {[j.category, SCHEDULE_LABELS[j.scheduleType] ?? j.scheduleType, j.location].filter(Boolean).join(' · ')}
                    </p>
      ...
      {selectedJob && (
        <JobDetailPanel
          ...
          onEdit={j => { startEdit(j); setSelectedJob(null); }}
          onDelete={id => { remove(id); setSelectedJob(null); }}
